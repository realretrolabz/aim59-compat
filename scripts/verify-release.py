...
bundle_root = PurePosixPath(f"rrlzAIM-{version}")
allowed_dlls = {
...
version_check = subprocess.run(
    [sys.executable, str(dist / "rrlzAIMlinux.pyz"), "--version"],
    check=True,
    capture_output=True,
    text=True,
)
if version_check.stdout.strip() != f"rrlzAIMlinux {version}":
...
with tarfile.open(archive, "r:gz") as source:
    ...
    launcher_path = bundle_root / "rrlzAIMlinux"
    ...
    expected_internal_sums = {"rrlzAIMlinux": digest_bytes(launcher)}
